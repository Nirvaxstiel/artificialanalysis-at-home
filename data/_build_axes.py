...
The catalog defines what axes exist; the projection engine (in _project_axes.py)
...
#   label: str,
#   source: str,
#   type: "pricing" | "quality" | "performance" | "meta",
#   unit: str | None,
#   higher_is_better: bool,
#   description: str,
#   range: [min, max] | null,         # observed range
#   models_have: int,                  # how many models have this axis
#   extract_path: [str,...],           # how to get value from model dict
#   aliases: [str,...],                # other common names
# }
...
# Cost segments (breakdown) sub-axes
...
# Detect actual category names from data
lb_orig_keys = set()
for m in models:
    sec = m
...
# Build lowercase→original mapping for LiveBench categories
# Source data keys: 'Mathematics', 'Coding', 'Language', 'Data Analysis', 'Agentic Coding', 'Reasoning', 'IF'
lb_lower_to_orig = {}
...
# Now build axes using lowercase IDs
...
for cat in lb_cats:
    ...
    axes.append({
        ...
        "group": "LiveBench",
        "_dict_key": orig_key,  # original mixed-case key for extraction
    })
...
# Sort by source, then type, then models_have (desc)
axes.sort(key=lambda a: (a["source"], a["type"], -a["models_have"]))

# Add composite min/max normalization hints
for a in axes:
    if a["range"] and a["range"][0] != a["range"][1]:
        a["normalize_range"] = [0, 100]  # standard normalization target
    else:
        a["normalize_range"] = None

# Add source-groups for easy filtering
source_groups = {}
for a in axes:
    g = a["source"]
...
def get_value(m, aid):
    """Extract axis value from model dict using axis id path notation."""
    parts = aid.split(".")
    source = parts[0]
    if source == "meta":
...
# For each pair of source_groups, show how many models have both
