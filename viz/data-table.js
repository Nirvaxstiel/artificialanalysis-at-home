...
(function() {
  ...
  const VIEWS = {
    ...
    providerData: {
      ...
      cols: [
        ...
        { key: 'hit_rate', label: 'CACHE HIT %', render: r => r.hit_rate != null ? r.hit_rate.toFixed(1) + '%' : '—', cls: 'num' },
        { key: 'eff_input_price', label: 'EFF IN $/M', render: r => r.eff_input_price != null ? window.VIZ_NUM.fmtUSD(r.eff_input_price) : '—', cls: 'num' },
        { key: 'eff_output_price', label: 'EFF OUT $/M', render: r => r.eff_output_price != null ? window.VIZ_NUM.fmtUSD(r.eff_output_price) : '—', cls: 'num' },
        { key: 'max_hit', label: 'MAX (ALL PROV)', render: r => r.max_hit != null ? r.max_hit.toFixed(1) + '%' : '—', cls: 'num' },
      ],
  ...
  function buildShell(container, view, viewKey, data) {
    ...
    if (viewDef.options) {
      ...
      html += `<div class="dt-opt-btns" data-opt-key="${viewDef.options.key}" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:6px;">`;
      for (const o of opts) {
        const active = o.name === defaultOpt ? ' active' : '';
        html += `<button class="dt-opt-btn${active}" data-opt="${o.name}">${o.name} <span class="dt-opt-count">(${o.count})</span></button>`;
      }
      html += '</div>';
    }
